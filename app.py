...
st.markdown("""
    ...
    /* MATCHING CHAT INPUT MESSAGE BAR */
    ...
    /* Force typing text, placeholders, and send button parameters */
...
if user_input := st.chat_input("Ask Ammars_GPT..."):
    ...
    with st.chat_message("assistant"):
        ...
        # Protected choices list parsing logic to stop extraction errors
        def parse_stream():
            for chunk in completion:
                if hasattr(chunk, 'choices') and chunk.choices:
                    first_choice = chunk.choices[0]
                    if hasattr(first_choice, 'delta') and first_choice.delta.content is not None:
                        yield first_choice.delta.content
